...
def shell(request: pytest.FixtureRequest, tmp_path: Path) -> Iterator[CommandShell]:
    ...
    version_args = (
        ...
        if name == "pwsh"
        else ["--version"] if name != "sh" else ["-c", "echo POSIX-sh"]
    )
...
def test_documented_headless_commands(shell: CommandShell) -> None:
    ...
    for record in devices:
        ...
        assert math.isclose(document["simulation"]["time"], 2.0)
    ...
    assert math.isclose(initial["simulation"]["time"], 0.2)
    assert math.isclose(resumed["simulation"]["time"], 0.4)
    assert resumed["provenance"]["resume"]["sha256"] == hashlib.sha256(
    ...
    subprocess.run(
        [
            sys.executable, "-m", "microsimulator", "run", "--model",
            str(shell.cwd / "results/tutorial models/biophysics.py"),
            "--parameter", 'scenario="basics"', "--seed", "42", "--steps", "20",
            "--dt", "0.02", "--output", str(shell.cwd / "uninterrupted.json"),
        ],
    ...
    rejected = subprocess.run(
        arguments, env=environment, cwd=shell.cwd, capture_output=True, text=True,
    ...
    async def exercise() -> None:
        for identifier in ("live-trap", "live-basics", "live-resume"):
            ...
            process = await asyncio.create_subprocess_exec(
                *arguments, cwd=shell.cwd, env=environment,
            ...
            try:
                ...
                async with ClientSession() as client, client.ws_connect(
                ...
                shell.records.append(
                    {"id": identifier, "command": BLOCKS[identifier],
                     "exit_code": process.returncode, "stopped": stopped,
...
def test_tutorial_flags_paths_and_links() -> None:
    documents = [
        *sorted((ROOT / "docs/tutorials").glob("*.md")),
        ROOT / "environments/metal/README.md", ROOT / "environments/cuda/README.md",
        ROOT / "viewer/README.md", ROOT / "docs/development/tutorial-command-validation.md",
    ]
    parser = _parser()
    checked = 0
